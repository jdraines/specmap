...
func (s *Store) UpsertPull(ctx context.Context, p *models.PullRequest) (*models.PullRequest, error) {
	row := s.Pool.QueryRow(ctx, `
		INSERT INTO pull_requests (repository_id, number, title, state, head_branch, base_branch, head_sha, author_login, spec_coverage)
	...
	`, p.RepositoryID, p.Number, p.Title, p.State, p.HeadBranch, p.BaseBranch, p.HeadSHA, p.AuthorLogin, p.SpecCoverage)
...
func (s *Store) ListPullsByRepo(ctx context.Context, repoID int64) ([]models.PullRequest, error) {
	rows, err := s.Pool.Query(ctx, `
		SELECT id, repository_id, number, title, state, head_branch, base_branch,
		       head_sha, author_login, spec_coverage, created_at, updated_at
		FROM pull_requests
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.RepositoryID, &p.Number, &p.Title, &p.State,
			&p.HeadBranch, &p.BaseBranch, &p.HeadSHA, &p.AuthorLogin, &p.SpecCoverage,
